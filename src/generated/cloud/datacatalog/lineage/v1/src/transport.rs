...
impl super::stub::Lineage for Lineage {
    async fn process_open_lineage_run_event(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}:processOpenLineageRunEvent", arg1,))
            })
            .ok_or_else(|| {
        ...
        let path = None
            ...
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).and_then(|m| m.process.as_ref()).map(|m| &m.name),
                        "process.name",
                        &[
                            Segment::Literal("projects"),
                            Segment::Literal("/"),
    ...
    async fn update_run(
        ...
        let path = None
            .or_else(|| {
                let arg1 = Some(&req).and_then(|m| m.run.as_ref()).map(|m| &m.name)?;
                if !matches(
                    arg1,
                    &[
                        Segment::Literal("projects"),
            ...
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).and_then(|m| m.run.as_ref()).map(|m| &m.name),
                        "run.name",
                        &[
                            Segment::Literal("projects"),
                            Segment::Literal("/"),
    ...
    async fn create_lineage_event(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/lineageEvents", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn search_links(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}:searchLinks", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn batch_search_link_processes(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}:batchSearchLinkProcesses", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
