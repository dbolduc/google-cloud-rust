...
impl super::stub::Datastream for Datastream {
    ...
    async fn discover_connection_profile(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/connectionProfiles:discover", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
        ...
        let path = None
            ...
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).and_then(|m| m.stream.as_ref()).map(|m| &m.name),
                        "stream.name",
                        &[
                            Segment::Literal("projects"),
                            Segment::Literal("/"),
    ...
    async fn lookup_stream_object(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/objects:lookup", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn start_backfill_job(
        ...
        let path = None
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.object)?;
                if !matches(
                    arg1,
                    &[
                        Segment::Literal("projects"),
                ...
                Some(format!("/v1/{}:startBackfillJob", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn stop_backfill_job(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}:stopBackfillJob", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
