...
impl super::stub::NotebookService for NotebookService {
    ...
    async fn check_instance_upgradability(
        ...
        let path = None
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.notebook_instance)?;
                if !matches(
                    arg1,
                    &[
                        Segment::Literal("projects"),
                ...
                Some(format!("/v2/{}:checkUpgradability", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).map(|m| &m.notebook_instance),
                        "notebook_instance",
                        &[
                            Segment::Literal("projects"),
                            Segment::Literal("/"),
