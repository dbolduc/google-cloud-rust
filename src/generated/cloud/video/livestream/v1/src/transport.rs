...
impl super::stub::LivestreamService for LivestreamService {
    ...
    async fn update_input(
        ...
        let path = None
            .or_else(|| {
                let arg1 = Some(&req).and_then(|m| m.input.as_ref()).map(|m| &m.name)?;
                if !matches(
                    arg1,
                    &[
                        Segment::Literal("projects"),
            ...
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).and_then(|m| m.input.as_ref()).map(|m| &m.name),
                        "input.name",
                        &[
                            Segment::Literal("projects"),
                            Segment::Literal("/"),
    ...
    async fn create_dvr_session(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/dvrSessions", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn update_pool(
        ...
        let path = None
            .or_else(|| {
                let arg1 = Some(&req).and_then(|m| m.pool.as_ref()).map(|m| &m.name)?;
                if !matches(
                    arg1,
                    &[
                        Segment::Literal("projects"),
            ...
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).and_then(|m| m.pool.as_ref()).map(|m| &m.name),
                        "pool.name",
                        &[
                            Segment::Literal("projects"),
                            Segment::Literal("/"),
