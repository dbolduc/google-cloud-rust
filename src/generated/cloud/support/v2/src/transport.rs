    ...
    async fn list_attachments(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v2/{}/attachments", arg1,))
            })
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.parent)?;
                if !matches(
...
impl super::stub::CaseService for CaseService {
    ...
    async fn search_cases(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v2/{}/cases:search", arg1,))
            })
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.parent)?;
                if !matches(
    ...
    async fn update_case(
        ...
        let path = None
            .or_else(|| {
                let arg1 = Some(&req).and_then(|m| m.case.as_ref()).map(|m| &m.name)?;
                if !matches(
                    arg1,
                    &[
                        Segment::Literal("projects"),
    ...
    async fn search_case_classifications(
        ...
        let path = None
            .or_else(|| Some(format!("/v2/caseClassifications:search",)))
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
