        ...
        let path = None
            ...
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).and_then(|m| m.posture.as_ref()).map(|m| &m.name),
                        "posture.name",
                        &[
                            Segment::Literal("organizations"),
                            Segment::Literal("/"),
    ...
    async fn extract_posture(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/postures:extract", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn list_posture_deployments(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/postureDeployments", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn list_posture_templates(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/postureTemplates", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
