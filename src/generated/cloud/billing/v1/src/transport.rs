...
impl super::stub::CloudBilling for CloudBilling {
    ...
    async fn list_billing_accounts(
        ...
        let path = None
            .or_else(|| Some(format!("/v1/billingAccounts",)))
            .or_else(|| {
                ...
                Some(format!("/v1/{}/billingAccounts", arg1,))
            })
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.parent)?;
                if !matches(
                ...
                Some(format!("/v1/{}/subAccounts", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn get_project_billing_info(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/billingInfo", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    async fn move_billing_account(
        ...
        let path = None
            ...
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.destination_parent)?;
                let arg2 = Some(&req).map(|m| &m.name)?;
                if !matches(
                    arg1,
                    &[
                        Segment::Literal("organizations"),
