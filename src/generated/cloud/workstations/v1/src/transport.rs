...
impl super::stub::Workstations for Workstations {
        ...
        let builder = None
            .or_else(|| {
                ...
                let builder = (|| {
                    let builder = self.inner.builder(reqwest::Method::POST, path);
                    let builder =
                        builder.query(&[("workstationConfigId", &req.workstation_config_id)]);
                    let builder = builder.query(&[("validateOnly", &req.validate_only)]);
                    Ok(builder)
                })();
                Some(builder)
    ...
    async fn update_workstation(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match2(Some(&req).and_then(|m| m.workstation.as_ref()).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/workstationClusters/"), Segment::SingleWildcard, Segment::Literal("/workstationConfigs/"), Segment::SingleWildcard, Segment::Literal("/workstations/"), Segment::SingleWildcard])?,
            );
    ...
    async fn generate_access_token(
        ...
        .or_else(|| {
            let path = format!(
                ...
                try_match2(Some(&req).map(|m| &m.workstation).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/workstationClusters/"), Segment::SingleWildcard, Segment::Literal("/workstationConfigs/"), Segment::SingleWildcard, Segment::Literal("/workstations/"), Segment::SingleWildcard])?,
            );
    ...
    async fn set_iam_policy(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}:setIamPolicy",
                try_match2(Some(&req).map(|m| &m.resource).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard, Segment::Literal("/workstationClusters/"), Segment::SingleWildcard, Segment::Literal("/workstationConfigs/"), Segment::SingleWildcard])?,
            );
