...
impl super::stub::AnalyticsHubService for AnalyticsHubService {
    async fn list_data_exchanges(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/dataExchanges", arg1,))
            })
            .ok_or_else(|| {
    ...
    async fn list_org_data_exchanges(
        ...
        let path = None
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.organization)?;
                if !matches(
                    arg1,
                    &[
        ...
        let path = None
            ...
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
                    let builder = PathMismatchBuilder::default();
                    let builder = builder.maybe_add_match_error(
                        Some(&req).and_then(|m| m.listing.as_ref()).map(|m| &m.name),
                        "listing.name",
                        &[
                            Segment::Literal("projects"),
                            Segment::Literal("/"),
    ...
    async fn list_subscriptions(
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}/subscriptions", arg1,))
            })
            .ok_or_else(|| {
                let mut paths = Vec::new();
                {
    ...
    ) -> Result<gax::response::Response<crate::model::ListSharedResourceSubscriptionsResponse>>
    {
        ...
        let path = None
            .or_else(|| {
                ...
                Some(format!("/v1/{}:listSubscriptions", arg1,))
            })
            .or_else(|| {
                let arg1 = Some(&req).map(|m| &m.resource)?;
                if !matches(
