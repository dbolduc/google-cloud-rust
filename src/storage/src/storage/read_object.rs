...
pub struct ReadObject<C = Crc32c> {
    stub: std::sync::Arc<dyn stub::dynamic::Storage>,
...
impl ReadObject<Crc32c> {
    pub(crate) fn new<B, O>(stub: std::sync::Arc<dyn stub::dynamic::Storage>, options: RequestOptions, bucket: B, object: O) -> Self
    where
        B: Into<String>,
        O: Into<String>,
    {
    ...
    C: Clone + ChecksumEngine + Send + Sync + 'static,
{
    ...
    pub async fn send(self) -> Result<impl ReadObjectResponse> {
        self.stub.read_object(self.request, self.options)
    }
}
