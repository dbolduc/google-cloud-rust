...
mod tests {
    ...
    async fn useful_binding_error() -> anyhow::Result<()> {
        ...
        // };
        // ```
        let e = client.delete_bucket().send().await.expect_err("Should fail locally with a binding error.");
        ...
        // };
        // ```
        let e = client.delete_folder().send().await.expect_err("Should fail locally with a binding error.");
        assert!(e.is_binding(), "{e:?}");
