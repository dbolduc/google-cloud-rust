...
use crate::{
    Result,
    google::{self, storage::control::v2::Folder},
};
...
impl super::stub::StorageControl for StorageControl {
    ...
    async fn rename_folder(
        ...
        let response: tonic::Response<crate::google::longrunning::Operation> = self
            .inner
            .execute(
                extensions,
                path,
        ...
        let op = op_from_proto::<
            crate::model::RenameFolderMetadata,
            crate::model::Folder,
            crate::google::storage::control::v2::RenameFolderMetadata,
            crate::google::storage::control::v2::Folder,
        >(grpc_op)?;
...
fn any_from_proto<T, PT>(any: prost_types::Any) -> Result<wkt::Any>
where
    T: wkt::message::Message + serde::Serialize + serde::de::DeserializeOwned,
    PT: gaxi::prost::FromProto<T> + prost::Name + Default,
{
    if any.type_url != T::typename() {
        panic!("bad metadata type");
    }
    let pt = any.to_msg::<PT>().unwrap();
    let t = pt.cnv();
    Ok(wkt::Any::try_from(&t).unwrap())
}

fn op_from_proto<M, R, MP, RP>(
    op: crate::google::longrunning::Operation,
) -> Result<longrunning::model::Operation>
where
    M: wkt::message::Message + serde::Serialize + serde::de::DeserializeOwned,
    R: wkt::message::Message + serde::Serialize + serde::de::DeserializeOwned,
    MP: gaxi::prost::FromProto<M> + prost::Name + Default,
    RP: gaxi::prost::FromProto<R> + prost::Name + Default,
{
    use crate::google::longrunning::operation::Result as U;
    use gaxi::prost::FromProto;
    use longrunning::model::operation::Result as T;
    let result = op.result.map(|result| match result {
        U::Error(v) => T::Error(v.cnv().into()),
        U::Response(any) => T::Response(any_from_proto::<R, RP>(any).unwrap().into()),
    });

    Ok(longrunning::model::Operation::new()
        .set_name(op.name)
        .set_metadata(op.metadata.map(|any| any_from_proto::<M, MP>(any).unwrap()))
        .set_done(op.done)
        .set_result(result))
}
