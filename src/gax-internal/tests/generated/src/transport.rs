...
impl super::stub::TestService for TestService {
    async fn do_foo(
        ...
        .or_else(|| {
            let path = format!(
                "/v2/{}:create",
                try_match2(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/locations/"), Segment::SingleWildcard])?,
            );
        ...
        .or_else(|| {
            let path = format!(
                ...
                try_match2(Some(&req).map(|m| &m.project).map(|s| s.as_str()), &[Segment::SingleWildcard])?,
                try_match2(Some(&req).map(|m| &m.location).map(|s| s.as_str()), &[Segment::SingleWildcard])?,
        ...
        .or_else(|| {
            let path = format!(
                ...
                try_match2(Some(&req).and_then(|m| m.child.as_ref()).map(|m| &m.project).map(|s| s.as_str()), &[Segment::SingleWildcard])?,
                try_match2(Some(&req).and_then(|m| m.child.as_ref()).map(|m| &m.location).map(|s| s.as_str()), &[Segment::SingleWildcard])?,
                try_match2(Some(&req).and_then(|m| m.child.as_ref()).map(|m| &m.id), &[Segment::SingleWildcard])?,
            );
