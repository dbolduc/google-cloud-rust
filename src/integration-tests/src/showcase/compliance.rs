...
use crate::Result;
use gaxi::path_parameter::{BindingError, SubstitutionFail, SubstitutionMismatch};
...
async fn binding_error_unset(client: &showcase::client::Compliance) -> Result<()> {
    ...
    let missing_fields: Vec<&String> = b.paths[0].subs.iter().map(|sub| &sub.field_name).collect();
    assert_eq!(
        missing_fields,
        vec![
            "info.f_string",
