...
use crate::Result;
use crate::model::{
    ArrowRecordBatch, BatchCommitWriteStreamsResponse, FinalizeWriteStreamResponse, ProtoRows,
};
...
impl PendingWriter<ArrowFormat> {
...
impl PendingWriter<ProtoFormat> {
    /// Appends rows to the pending stream.
...
mod tests {
    ...
    async fn arrow_request_fields() -> anyhow::Result<()> {
        ...
        let writer = PendingWriter::new(transport, write_stream(), ArrowFormat::new(schema()));
    ...
    async fn proto_request_fields() -> anyhow::Result<()> {
        let transport = Arc::new(test_transport("http://ignored:1").await?);
        let writer =
            PendingWriter::new(transport, write_stream(), ProtoFormat::new(proto_schema()));
        assert_eq!(writer.write_stream(), write_stream());
