...
async fn run_stream_task(inner: Arc<Transport>, mut req_rx: mpsc::UnboundedReceiver<WriteRequest>) {
    ...
    let Stream {
        ...
        Err(e) => {
            ...
            // Dropping `req_rx` causes any writes queued concurrently while the stream was
            // connecting to receive `RecvError`, which maps to `AppendError::UnexpectedEndOfStream`.
            // The dispatcher's retry loop will catch this and retry on the replacement stream.
            return;
        }
    };
