...
use crate::model::{AppendRowsRequest, ArrowRecordBatch, ProtoRows};
use std::sync::Arc;
...
impl<F> DefaultWriter<F> {
    pub(crate) fn new(pool: Arc<StreamPool>, write_stream: String, format: F) -> Self {
...
impl DefaultWriter<ArrowFormat> {
...
impl DefaultWriter<ProtoFormat> {
    /// Append rows to the stream.
...
mod tests {
    ...
    async fn arrow_request_fields() -> anyhow::Result<()> {
        ...
        let writer = DefaultWriter::new(pool, write_stream(), ArrowFormat::new(schema()));
        ...
        let b = writer.append(arrow_rows(2));
        assert_eq!(b.req.write_stream, write_stream());
        let data = b.req.arrow_rows().expect("arrow rows should be set");
        let s = data.writer_schema.as_ref().expect("schema should be set");
        assert_eq!(s.serialized_schema, "test");
    ...
    async fn proto_request_fields() -> anyhow::Result<()> {
        let transport = Arc::new(test_transport("http://ignored:1").await?);
        let pool = Arc::new(StreamPool::new(transport, StreamPoolOptions::default()));
        let writer = DefaultWriter::new(pool, write_stream(), ProtoFormat::new(proto_schema()));
    ...
    #[tokio::test]
    async fn proto_basic_success() -> anyhow::Result<()> {
        let (response_tx, response_rx) = mpsc::channel(10);
        ...
        response_tx.send(Ok(convert(&test_response(1)))).await?;
        let resp = writer.append(proto_rows(1)).send().await?;
        assert_eq!(resp.offset, Some(1));
