...
pub(super) mod tests {
    ...
    async fn pending_extends_size_management() {
        ...
        // `LeaseState::next_event()`. If we decide to clean up the pending
        // extends elsewhere, this test will need an update.
        assert!(
            state.pending_extends.is_empty(),
            "The lease extension task should have completed. We should not hold onto it."
        );
    }
