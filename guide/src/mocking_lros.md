...
```rust,ignore
{{#rustdoc_include ../samples/tests/mocking_lros.rs:auto-mockall-macro}}
```
...
The `BatchRecognize` RPC returns partial results in the form of a `speech::model::OperationMetadata`. Like before, we will need to pack this into the returned `Operation`, but this time into the `Operation::metadata` field.
...
First we need to update our `MockSpeech` stub to override `get_operation()`. We will see why shortly.
...
1. The first expectation is set on `batch_recognize()`, whereas all subsequent expectations are set on `get_operation()`.
    ...
    From then on, the client library just checks the status of that LRO. It does this using the `GetOperation` RPC.
...
2. Expectations are set in a [sequence].
    ...
    This is a particularity of `mockall`. It allows `mockall` to distinguish between different expectations.
...
As before, simulating errors is no different than simulating a successful response.

```rust,ignore
{{#rustdoc_include ../samples/tests/mocking_lros.rs:error}}
```

Errors can be returned from the original RPC (`batch_recognize`) or from subsequent `get_operation` calls.

______________________________________________________________________

## Automatic polling - Full test

Putting all this code together into a full test looks as follows:

```rust,ignore,noplayground
{{#include ../samples/tests/mocking_lros.rs:auto-all}}
```

## Manual polling with intermediate metadata - Full test

```rust,ignore,noplayground
{{#include ../samples/tests/mocking_lros.rs:manual-all}}
```

## Simulating errors - Full test

TODO : maybe omit this. It is not that valuable, tbh.

```rust,ignore,noplayground
{{#include ../samples/tests/mocking_lros.rs:error-all}}
```
